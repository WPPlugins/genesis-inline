<?php
if ( !function_exists( 'returner' ) ) {
	function returner( $value ) {
        return create_function( '', 'return '.var_export( $value, true ).';' );
	}
}

if ( !function_exists( 'lambda' ) ) {
function lambda( $args, $expression, $locals = array() ) {
        $export_call = $locals? 'extract( '.var_export( $locals, true ).', EXTR_PREFIX_SAME, "ext");' : '';
        return create_function( $args, $export_call.' return '.$expression.';' );
}
}

class GInline {
	function init() {
		add_filter( 'the_content', 'make_clickable' );

		if ( isset( $_REQUEST['GI-ajax'] ) ) {
			require_once( GENESISINLINE_LIB . 'ajax.php' );
			GInlineAjax::dispatch();
			die();
		}

		if ( !current_user_can( 'upload_files' ) )
			return;

		$is_media_upload = isset( $_REQUEST['GI-upload'] );

		// don't redirect to https version when uploading files, since the domain may be different
		// and we don't have SSL certificates for blog domain, only for admin
		if ( $is_media_upload && isset( $GLOBALS['pagenow'] ) && 'media-upload.php' == $GLOBALS['pagenow'] ) {
			force_ssl_admin( is_ssl() );
			add_filter( 'get_user_option_use_ssl', returner( false ) );
		}

		if ( $is_media_upload ) {
			add_filter( 'flash_uploader', returner( false) );
			add_filter( 'auth_redirect_scheme', returner( 'logged_in' ) );
			add_filter( 'admin_url', array( 'GInline', 'url_filter' ) );
			add_filter( 'includes_url', array( 'GInline', 'url_filter' ) );
			add_filter( 'script_loader_src', array( 'GInline', 'url_filter' ) );
			add_filter( 'wp_get_attachment_url', lambda( '$url', 'str_replace(get_bloginfo("url")."/", site_url("/"), $url);' ), 11 );
			add_filter( 'media_upload_form_url', lambda( '$url', 'add_query_arg( array( "GI-upload" => "true" ), $url );' ) );
		}
	}

	function media_buttons() {
		include_once ABSPATH . '/wp-admin/includes/media.php';
		ob_start();
		do_action( 'media_buttons' );
		return self::make_media_urls( ob_get_clean() );
	}

	/**
	 * Make sure the URL is loaded from the same domain as the frontend
	 */
	function url_filter( $url, $path = '' ) {
		$parsed = parse_url( $url );
		$host = ( isset( $parsed['host'] ) ) ? $parsed['host'] : '';
		if (!false === strpos( 'http', $url) )
			return preg_replace( '|https?://'.preg_quote( $host ).'|', get_bloginfo( 'url' ), $url );
		return $url;
	}

	function admin_url( $path ) {
		return self::url_filter( admin_url( $path ) );
	}

	function make_media_urls( $string ) {
		return str_replace( 'media-upload.php?', self::admin_url( 'media-upload.php?GI-upload=true&' ), $string );
	}
	function user_avatar( $args = '' ) {
		$current_user = wp_get_current_user();

		$defaults = array(
			'user_id' => false,
			'email' => ( isset( $current_user->user_email ) ) ? $current_user->user_email : '',
			'size' => 48,
			'echo' => true
		);

		$args = apply_filters( 'genesis_inline_user_avatar_args', $args );
		$r = wp_parse_args( $args, $defaults );
		extract( $r, EXTR_SKIP );

		if ( !$user_id )
			$avatar = get_avatar( $email, $size );
		else
			$avatar = get_avatar( $user_id, $size );

	 	$user_avatar = apply_filters( 'genesis_inline_user_avatar', $avatar, $r );
	 	
	 	if( !$echo )
	 		return $user_avatar;
	 		
	 	echo $user_avatar;
	}
}
