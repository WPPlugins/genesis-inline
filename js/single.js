			...
			ajaxTags = $.get(queryString, function(suggest) {
				...
				$.each(tag_list, function(key,value) {
					if($.inArray(value, tags) == -1) {
						$('#tag-suggest').append('<div class="tag-suggest" rel="'+value+'"><span class="u-suggest">'+value.substr(0, maybe.length)+'</span>'+value.substr(maybe.length)+'</div>');
						c += 1;
					}
				});
				if(!c)
...
//*/
		...
		ajaxCheckPosts = $.getJSON(queryString, function(newPosts){
			if (newPosts != null) {
				...
				if (!isFirstFrontPage || (typeof newPosts.html == "undefined") ) {
					...
					var newUpdatesLi = $("#content ul#inline-sleeve > li:first");
					newUpdatesLi.hide().slideDown(1000, function() {
						$(this).attr('style', '');
					});
					var counter = 0;
	...
	*/
	function newPost(trigger) {
		...
		var args = {action: 'new_post', _ajax_post:nonce, posttext: posttext, tags: tags, post_format: post_format, post_title: post_title, post_citation: post_citation, post_status: post_status};
		var errorMessage = '';
		$.ajax({
			type: "POST",
			url: ajaxUrl,
